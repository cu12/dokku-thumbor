...
@test "($PLUGIN_COMMAND_PREFIX:link) error when the service is already linked to app" {
  ...
  assert_contains "${lines[*]}" "Already linked as THUMBOR_URL"
}

@test "($PLUGIN_COMMAND_PREFIX:link) exports THUMBOR_URL to app" {
  dokku "$PLUGIN_COMMAND_PREFIX:link" l my_app
  url=$(dokku config:get my_app THUMBOR_URL)
  assert_contains "$url" "http://dokku-thumbor-l:8000"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when THUMBOR_URL already in use" {
  dokku config:set my_app THUMBOR_URL=http://host:8000
  dokku "$PLUGIN_COMMAND_PREFIX:link" l my_app
  run dokku config my_app
  assert_contains "${lines[*]}" "DOKKU_THUMBOR_"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.thumbor.l:dokku-thumbor-l"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}
