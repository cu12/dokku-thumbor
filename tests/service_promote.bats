...
@test "($PLUGIN_COMMAND_PREFIX:promote) error when the service is already promoted" {
  ...
  assert_contains "${lines[*]}" "already promoted as THUMBOR_URL"
}

@test "($PLUGIN_COMMAND_PREFIX:promote) changes THUMBOR_URL" {
  dokku config:set my_app "THUMBOR_URL=http://host:8000" "DOKKU_THUMBOR_BLUE_URL=http://dokku-thumbor-l:8000"
  dokku "$PLUGIN_COMMAND_PREFIX:promote" l my_app
  url=$(dokku config:get my_app THUMBOR_URL)
  assert_equal "$url" "http://dokku-thumbor-l:8000"
}
